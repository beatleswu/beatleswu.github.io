...
class CoinPurchaseResult:
    ...
    def from_payload(
        ...
        *,
        ...
        ownership_result = payload["ownership_result"]
        if not isinstance(ownership_result, Mapping):
            raise SchemaUnavailable("committed ownership result is not recoverable")
        return cls(
            ...
            ownership_result=dict(ownership_result),
            ...
            replayed=replayed,
            ownership_reference=_payload_ownership_reference(payload),
        )


def _normalize_player_inventory_reference(value: Any) -> str | None:
    if value is None:
        return None
    if not isinstance(value, str):
        raise SchemaUnavailable("ownership_reference must be text")
    prefix, separator, raw_row_id = value.partition(":")
    if prefix != "player_inventory" or separator != ":" or not raw_row_id.isdigit():
        raise SchemaUnavailable("ownership_reference has an invalid format")
    row_id = int(raw_row_id)
    if row_id <= 0 or str(row_id) != raw_row_id:
        raise SchemaUnavailable("ownership_reference row id must be canonical and positive")
    return value


def _payload_ownership_reference(payload: Mapping[str, Any]) -> str | None:
    top_level = _normalize_player_inventory_reference(payload.get("ownership_reference"))
    nested_value = payload.get("ownership_result", {}).get("ownership_reference")
    nested = _normalize_player_inventory_reference(nested_value)
    if top_level is not None and nested is not None and top_level != nested:
        raise SchemaUnavailable("top-level and nested ownership references disagree")
...
def _replay_or_fail(
    ...
    *,
    ...
    if status == "COMMITTED":
        result = CoinPurchaseResult.from_payload(
            existing.get("result_payload") or {}, replayed=True
        )
        if result.destination == "player_inventory" and result.ownership_reference is None:
            raise SchemaUnavailable(
                "committed player_inventory purchase has no exact ownership reference"
            )
        return result
...
def _ownership_reference_for_row_id(row_id: Any) -> str:
    ...
    return f"player_inventory:{row_id}"


def _insert_player_inventory(
    conn: Any,
    *,
    user_id: int,
    item_id: str,
    canonical_slot: str | None,
    has_canonical_slot: bool,
    obtained_at: Any,
) -> str:
    """Insert one ownership row and capture its own primary key at INSERT time."""
    ...
    if has_canonical_slot:
        ...
        parameters = (user_id, item_id, canonical_slot, obtained_at, "coin_shop")
    else:
        statement = (
            "INSERT INTO player_inventory"
        ...
        parameters = (user_id, item_id, obtained_at, "coin_shop")

    try:
        if _is_sqlite(conn):
            inserted = conn.execute(statement, parameters)
            row_id = getattr(inserted, "lastrowid", None)
        else:
            inserted = conn.execute(f"{statement} RETURNING id", parameters)
        ...
        raise AcquisitionFailed("player_inventory acquisition insert failed") from exc
    return _ownership_reference_for_row_id(row_id)
    ...
    def _acquire_equipment_inventory(
        ...
        *,
        ...
        try:
            ...
            ownership_reference = _insert_player_inventory(
                conn,
                user_id=user_id,
                item_id=offer.item_id,
                canonical_slot=canonical_slot,
                has_canonical_slot=_player_inventory_has_column(
                    conn, CANONICAL_SLOT_COLUMN
                ),
                obtained_at=_timestamp(conn),
            )
...
def _validate_acquisition_outcome(
    ...
    *,
    ...
    if offer.destination == "player_inventory":
        try:
            expected = _normalize_player_inventory_reference(outcome.ownership_reference)
        except SchemaUnavailable as exc:
            raise AcquisitionFailed(
                "player_inventory acquisition returned an invalid ownership reference"
            ) from exc
        if expected is None:
            raise AcquisitionFailed(
                "player_inventory acquisition returned no exact ownership reference"
            )
    elif outcome.ownership_reference is not None:
        raise AcquisitionFailed(
            "non-player_inventory acquisition returned an ownership reference"
        )
...
def purchase_with_coins(
    ...
    *,
    ...
    result = CoinPurchaseResult(
        ...
        replayed=False,
        ownership_reference=provisional.ownership_reference,
    )
