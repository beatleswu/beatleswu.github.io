(function (root) {
  ...
  function ensureHost() {
    ...
    host.setAttribute('aria-label', 'SGF question report');
  ...
  async function submit() {
    ...
    status.className = 'sgf-report-status'; status.textContent = '送出中…';
    var payload = {};
    Object.keys(state.context).forEach(function (key) { payload[key] = state.context[key]; });
    ...
    try {
      ...
      status.textContent = '已送出，證據會交由管理員審核。';
      ...
      status.className = 'sgf-report-status error'; status.textContent = error.message || '回報失敗';
    }
  }
