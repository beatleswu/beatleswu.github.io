...
historical catch-up.  The caller must therefore pass the server-owned
``historical_catchup`` context explicitly; the transport never guesses it.
"""
...
def _historical_marker(outcome: Mapping[str, Any], value: Any) -> bool:
    """Require caller-owned context instead of guessing normal vs catch-up."""
    ...
    if value is _MISSING:
        _reject(
            ...
            "historical_catchup must be supplied by the server caller",
        )
...
def _validate_d030_outcome(
    ...
    if status == "NOT_ELIGIBLE":
        ...
        expected = {
            ...
            # D030 does not inspect pet_collection on the ineligible branch;
            # absence of eligibility is not proof of non-ownership.
            "already_owned": None,
            "replay": False,
            "eligible": False,
            "cleared": False,
...
def build_adventure_spirit_unlock_results(
    ...
    response to reach D031.  Sorting is presentation/transport stability;
    it does not select or authorize a milestone.
    """
...
def serialize_adventure_spirit_unlock_transport(
    ...
    """Serialize the response fragment deterministically for contract tests."""
