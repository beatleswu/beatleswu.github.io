...
# Run_Audition_Set_A.cmd on the Owner's local Windows machine.
#
...
Write-Host "=== E10 Zone 1：試聽比較集 (AUDITION SET A) ===" -ForegroundColor Cyan
Write-Host "本工具只會產生 16 個試聽比較檔（8 個角色/語言組合，各 2 個候選聲音）。"
...
try {
    ...
    Write-Host ""
    Write-Host "[2/2] 正在產生 16 個試聽比較檔（AUDITION SET A）..." -ForegroundColor Cyan
    Write-Host "（若資料夾內有先前產生的試聽檔，會先清除再重新產生，避免新舊檔案混在一起。）"
    & $PythonExe $PythonScript --audition-set-a
    if ($LASTEXITCODE -ne 0) {
        Fail "試聽檔案產生或驗證失敗，請往上捲動查看詳細錯誤訊息（例如檔案數量不足或某個檔案是空的）。" `
             "'--audition-set-a' exited with code $LASTEXITCODE."
    }
    $ExitedCleanly = $true
}
finally {
...
$AuditionDir = Join-Path $ScriptDir "_local_review\audition_set_a"
...
if ($Mp3Count -ne 16) {
    Fail "試聽資料夾內的檔案數量不是預期的 16 個（實際：$Mp3Count 個），請重新執行一次。" `
         "Expected 16 non-empty MP3 files, found $Mp3Count in $AuditionDir."
}

Write-Host ""
Write-Host "完成！已產生 16 個試聽比較檔，正在開啟資料夾..." -ForegroundColor Green
