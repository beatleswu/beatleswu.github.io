...
def test_player_inventory_requires_and_preserves_exact_committed_reference():
    raw, connection = _database()
    result, operation, lineage = _facts(destination="player_inventory")
    result["ownership_reference"] = "player_inventory:417"
    result["ownership_result"]["ownership_reference"] = "player_inventory:417"

    canonical = _run(connection, result=result, operation=operation, lineage=lineage)

    assert canonical.ownership_reference == "player_inventory:417"
