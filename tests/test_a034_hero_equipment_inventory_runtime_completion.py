...
This test suite exercises the current route path with a disposable legacy
SQLite inventory.  It intentionally leaves the schema unchanged: the live
default path is the existing server-owned ``player_inventory`` route writer.
"""
...
def test_acquire_backpack_equip_replace_unequip_and_reload_use_one_authority(
    ...
    reloaded_items = {
        item["item_id"]: item
        for item in reloaded.get("/api/player/inventory").get_json()
    }
