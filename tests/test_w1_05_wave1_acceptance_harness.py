...
def test_desktop_matrix_viewport_is_independently_pinned_to_1440x900() -> None:
    matrix = _load_json(MATRIX_PATH)
    desktop_row = next(
        row for row in matrix["rows"] if row["gate_id"] == "W1-05-J-DESKTOP"
    )
    assert desktop_row["test_or_check"] == (
        "Run the existing static fixture at 1440x900 in Chromium"
    )
