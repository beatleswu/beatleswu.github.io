...
def run_real_functions(tmp_path, output, *, exit_code=0, call_try=True):
    ...
    probe.write_text(
        ...
        "  return [ordered]@{ output = $script:FakeOutput; exit_code = $script:FakeExitCode; command = $Command }\n"
        "}\n"
...
def test_real_readiness_helper_accepts_stderr_diagnostics_merged_around_stdout_record(tmp_path):
    ...
    result = run_real_functions(tmp_path, diagnostic + "\n" + framed(readiness_report()) + "\n" + diagnostic)
    assert result["ok"] is True
    assert len(result["value"]["startup_diagnostics"]) == 2
