...
def validate_preview_identity(preview_identity, *, snapshot, snapshot_file, database_url):
    ...
    if preview_identity.get("environment_identity") != _environment_identity():
...
def summarize_post_grant_state(conn, *, board_type, period_key):
    ...
    item_rows = conn.execute(
        "SELECT reward_key, SUM(quantity)::int "
        "FROM leaderboard_reward_component_log cl "
        "JOIN leaderboard_reward_claims c ON c.id = cl.claim_id "
        "WHERE c.board_type = %s AND c.period_key = %s "
        "AND cl.component = 'item' AND cl.result = 'granted' "
        "GROUP BY reward_key",
        (board_type, period_key),
    ).fetchall()
    badge_rows = conn.execute(
        ...
        "JOIN leaderboard_reward_claims c ON c.id = cl.claim_id "
        "WHERE c.board_type = %s AND c.period_key = %s "
        "AND cl.component = 'badge' AND cl.result = 'granted' "
        "GROUP BY reward_key",
        (board_type, period_key),
    ).fetchall()
    return {
        ...
        "total_items": {str(row[0]): int(row[1]) for row in item_rows},
        "total_badges": {str(row[0]): int(row[1]) for row in badge_rows},
    }
