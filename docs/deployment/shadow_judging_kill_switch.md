...
./scripts/release/set-shadow-judging.ps1 -Operation validate-rollback -RollbackBackupId <exact-backup-id> -LayoutFile ./deploy/release-layout.production.json
```
...
./scripts/release/set-shadow-judging.ps1 -Operation rollback -RollbackBackupId <exact-backup-id> -LayoutFile ./deploy/release-layout.production.json -Execute -OwnerGate GO_SHADOW_ROLLBACK
```
...
and verifies recovery. Public `rollback` requires `-RollbackBackupId` and
selects only that exact checksum-valid backup; it never resolves "latest".
`validate-rollback` validates the same exact identity without mutation or lock
acquisition. A rollback first backs up the current state so a failed rollback can itself be
