...
| Admin/debug | No separate Monster admin/debug consumer found in the current source inventory | N/A | N/A | None | No adapter target | N/A | `/api/monster/status` is player initialization, not an admin/debug authority; no new endpoint is introduced. |
