...
The later route writer may attach the serialized list only after the
authoritative Adventure settlement has run D030's milestone catch-up through
the existing B023 `SPIRIT_UNLOCK` sink. D032 deliberately does not change the
...
| `already_owned` | True for D030 `NO_OP` and replayed `REPLAY`; `null` for `NOT_ELIGIBLE` because D030 does not inspect ownership on that branch. |
| `historical_catchup` | Explicit server-caller context. If D030 does not provide it and the caller does not provide it, the value is `null`; it is never guessed. |
...
The retained fields include `user_id`, exact `zone_key`, `zone_number`, the
D030-mapped `spirit_id`, D030 `operation_id`, `source_reference`, source
authority and fact, `operation_status`, `replayed`, mutation counts, zero
compensation and replacement counts, `ownership_store=pet_collection`, and
...
- `NOT_ELIGIBLE`: `ownership_created=false`, `already_owned=null`,
