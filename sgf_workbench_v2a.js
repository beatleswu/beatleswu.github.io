...
(function () {
  ...
  async function loadQuestion(index) {
    ...
    try {
      const query = new URLSearchParams({ filter: state.filter, search: state.search });
      const payload = await jsonFetch(`${API}/questions/${encodeURIComponent(index)}?${query}`);
      state.current = payload.question; state.selectedNode = "0"; state.replay = state.current?.playback || null;
      state.navigation = payload.navigation || state.navigation;
  ...
  function replayClient(tree, nodeId) {
    // Board states are generated by the canonical backend replay adapter.  Do
    // not maintain a second capture implementation in the browser.
    if (tree?.board_states && Object.prototype.hasOwnProperty.call(tree.board_states, nodeId)) {
      return { status: "PASS", node_id: String(nodeId), stones: (tree.board_states[nodeId] || []).map((stone) => ({...stone})) };
    }
    ...
    return { status: "FAIL", error: "backend_board_state_missing", node_id: String(nodeId), stones: [] };
  }
  ...
  function renderMeta() {
    ...
    document.getElementById("v2a-total").textContent = `${state.currentPosition || 0} / ${state.total || 0} · ${q.reviewed_record_sha256.slice(0, 10)}`;
  ...
  function formatGoCoordinate(x, y, size) {
    const columns = "ABCDEFGHJKLMNOPQRST";
    if (!Number.isInteger(x) || !Number.isInteger(y) || x < 0 || x >= columns.length || y < 0 || y >= size) return "?";
  ...
  function renderTree() {
    ...
    tree.innerHTML = nodes.map((node) => {
      ...
      const text = node.id === "0" ? "初始局面" : (move?.pass ? `${move.color} pass` : move?.invalid ? "無效手" : `${move?.color || "?"} ${move?.coordinate || formatGoCoordinate(move?.x, move?.y, state.current?.tree?.board_size || 19)}`);
