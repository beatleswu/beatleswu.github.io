...
* player_inventory:{exact_inserted_row_id}

player_inventory is accepted only when C026 has persisted the exact
inserted row reference in the committed result.
"""
...
SUPPORTED_DESTINATIONS = (SHOP_INVENTORY, PLAYER_WARDROBE, PLAYER_INVENTORY)
...
def _normalize_player_inventory_reference(value: Any) -> str:
    """Validate C026's exact, canonical inserted-row reference."""

    if not isinstance(value, str):
        _fail(
            "MALFORMED_OWNERSHIP_REFERENCE",
            "player_inventory ownership_reference must be text",
        )
    prefix, separator, raw_row_id = value.partition(":")
    if prefix != PLAYER_INVENTORY or separator != ":" or not raw_row_id.isdigit():
        _fail(
            "MALFORMED_OWNERSHIP_REFERENCE",
            "player_inventory ownership_reference has an invalid format",
        )
    row_id = int(raw_row_id)
    if row_id <= 0 or str(row_id) != raw_row_id:
        _fail(
            "MALFORMED_OWNERSHIP_REFERENCE",
            "player_inventory row id must be canonical and positive",
        )
    return value
...
def _verify_authority_row(conn: Any, *, destination: str, user_id: str, item_id: str) -> None:
    ...
    if destination == PLAYER_INVENTORY:
        # C026 captures the exact inserted row id in the committed result.
        # Reading player_inventory here would risk resolving a replay to a
        # different duplicate row and is intentionally forbidden.
        return
...
def adapt_committed_shop_purchase(
    ...
    if destination == PLAYER_INVENTORY:
        top_reference = result.get("ownership_reference", _MISSING)
        nested_reference = ownership.get("ownership_reference", _MISSING)
        references = [
            value
            for value in (top_reference, nested_reference)
            if value is not _MISSING and value is not None
        ]
        if not references:
            _fail(
                PLAYER_INVENTORY_FAILURE_CODE,
                "C026 committed result has no exact player_inventory ownership reference",
            )
        if len(references) == 2 and not _same_identity(references[0], references[1]):
            _fail(
                "OWNERSHIP_REFERENCE_MISMATCH",
                "top-level and nested ownership references disagree",
            )
        ownership_reference = _normalize_player_inventory_reference(references[0])
        reference_kind = "ROW_PRIMARY_KEY"

        lineage_ownership = _value(lineage_payload, "ownership_result")
        if lineage_ownership is not _MISSING:
            lineage_ownership = _json_mapping(
                lineage_ownership,
                "lineage_evidence.payload.ownership_result",
            )
            lineage_reference = lineage_ownership.get("ownership_reference", _MISSING)
            if lineage_reference is not _MISSING and lineage_reference is not None:
                normalized_lineage_reference = _normalize_player_inventory_reference(
                    lineage_reference
                )
                if normalized_lineage_reference != ownership_reference:
                    _fail(
                        "OWNERSHIP_REFERENCE_MISMATCH",
                        "D5A ownership evidence disagrees with the committed result",
                    )
    else:
        ownership_reference = (
        ...
        reference_kind = "COMPOSITE_AUTHORITY_KEY"
        supplied_reference = _first_present(result, ownership, "ownership_reference")
        if supplied_reference is not _MISSING and supplied_reference is not None:
            if not _same_identity(supplied_reference, ownership_reference):
                _fail(
    ...
    metadata: dict[str, Any] = {
        "bridge_version": "D024_SHOP_ACQUISITION_RESULT_BRIDGE_V1",
        "source_destination": destination,
        "ownership_reference_kind": reference_kind,
    }
    ...
    d018_destination = {
        SHOP_INVENTORY: "STACK_INVENTORY",
        PLAYER_WARDROBE: "PLAYER_WARDROBE",
        PLAYER_INVENTORY: "PLAYER_INVENTORY",
    }[destination]
