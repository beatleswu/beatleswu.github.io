...
def award_zone_star_from_authoritative_answer(
    conn: Any,
    user_id: int,
    zone_key: str,
    submission_id: str,
    ...
    """Atomically award at most one Zone star for one server-settled answer.

    The caller is responsible for proving that ``submission_id`` belongs to a
    server-settled, correct Adventure answer.  It is deliberately impossible
    for this function to receive a client grade, historical count, Boss clear,
    or reward/unlock instruction.
    """
    ...
    # The progress row is created only after the caller has presented a
    # server-owned event.  PostgreSQL gets a row lock; SQLite serializes the
    # write transaction in the test/runtime adapter.
    conn.execute(
    ...
    if _rowcount(inserted) != 1:
        # A concurrent or earlier event already owns this star number.  Read
        # the resulting authority; never manufacture a different star.
    ...
    conn.execute(
        ...
        (next_star, earned_at, uid, normalized_zone, next_star),
    )
    return {
        "status": "awarded",
        "zone_key": normalized_zone,
