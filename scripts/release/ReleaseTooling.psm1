...
function Initialize-WindowsPowerShellModulePath {
    <#
    Windows PowerShell 5.1 can inherit a PowerShell 7 PSModulePath from the
    release runner.  In that state built-in utility cmdlets such as
    Get-FileHash are not discoverable even though the Windows PowerShell
    module is installed.  Repair only this child process environment; do not
    modify the host environment or any product/runtime source.
    #>
    $requiredRoots = @()
...
function Invoke-Git {
    param(
        ...
        # Ordinary Git commands complete well inside this bound on the
        # release host.  Worktree creation/removal supplies the measured,
        # operation-specific 600 second bound below because checkout cost is
        # materially different from metadata/read commands.
        [int]$TimeoutSeconds = 60
    )
    # Invoke-BoundedNativeCommand is declared later in this module and is
    # resolved when Invoke-Git is called.  Routing Git through the same
    # process-tree-aware runner prevents a stuck index/worktree operation
    # from defeating the caller's watchdog and leaves finally blocks a chance
    # to clean up task-owned resources.
...
function Get-RegisteredGitWorktreePaths {
    <#
    Cleanup only needs linked-worktree identities.  `git worktree list`
    walks every registration and was the shared latency hotspot when the
    canonical checkout contained a large historical registry.  Git stores
    the exact linked-worktree path in each common .git/worktrees/*/gitdir
    file, so read those small metadata files directly.  A malformed or
    inaccessible foreign record is never an authorization to touch it.
    #>
...
function Remove-PartiallyCreatedDetachedWorktree {
    <#
    Roll back only the exact generated path when `git worktree add` fails
    before the normal generated-worktree ledger can be populated.  The
    parent, prefix, and reparse-point checks make a timeout cleanup
    task-owned and fail closed for substitutions or foreign paths.
    #>
    param(
        [Parameter(Mandatory = $true)][string]$Path,
    ...
    try {
        ...
        $candidateParentInfo = [System.IO.Directory]::GetParent($candidate)
        if ($null -eq $candidateParentInfo) {
            return
        }
        $candidateParent = Get-CanonicalFilesystemPath -Path $candidateParentInfo.FullName -Label 'Partial cleanup parent path'
        if (-not (Test-CanonicalPathEqual -Left $candidateParent -Right $parent)) {
            return
        }
        ...
        if ($registered.Count -eq 1) {
            try {
                Invoke-Git -Arguments @('worktree', 'remove', '--force', '--force', '--', $candidate) `
                    -WorkingDirectory $RepositoryRoot -TimeoutSeconds 600 | Out-Null
            }
            catch {
                return
            }
        ...
        # Keep an unresolved exact path for manual review rather than
        # broadening cleanup after any identity check fails.
    }
}
...
function New-DetachedWorktree {
    ...
    try {
        # Measured canonical checkout evidence showed worktree creation at
        # 372.8s under the current linked-worktree registry.  Keep this
        # operation-specific bound wide enough for legitimate checkout cost;
        # ordinary Git reads retain the 60s bound in Invoke-Git.
        Invoke-Git -Arguments @('worktree', 'add', '--detach', $worktree, $GitSha) `
            -WorkingDirectory $repositoryRoot -TimeoutSeconds 600 | Out-Null
...
function Remove-DetachedWorktree {
    ...
    try {
        Invoke-Git -Arguments @('worktree', 'remove', '--force', '--', $candidate) `
            -WorkingDirectory $record.repository_root -TimeoutSeconds 600 | Out-Null
    }
    catch {
        # A timeout can interrupt Git while it is unwinding a checkout.  Only
        # that timeout case may retry the exact ledger-owned path with Git's
        # documented double-force override.  Ordinary policy errors (notably
        # an intentionally locked worktree) must remain fail-closed and leave
        # the path for review.
        if (-not ([string]$_.Exception.Message).StartsWith('Timed out after ')) {
            throw
        }
        Invoke-Git -Arguments @('worktree', 'remove', '--force', '--force', '--', $candidate) `
            -WorkingDirectory $record.repository_root -TimeoutSeconds 600 | Out-Null
